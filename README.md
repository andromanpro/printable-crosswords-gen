...
🟢 **Демо:** https://andromanpro.github.io/printable-crosswords-gen/
...
Корпус в репозитории — это **пример**. Если хочешь — форкни, замени `data/` на свой (литература, кулинария, IT — что угодно), и получишь генератор кроссвордов под свою тему. Алгоритм тематически нейтрален.
...
Корпус — обычные JS-файлы в `data/`:
- `words.js` — базовый корпус.
- `pack-*.js` — тематические паки, опционально подключаемые в UI.

Чтобы подставить свой корпус, скажем «Литература»:

1. Создай файл `data/pack-literature.js`:
...
2. Подключи в `index.html` (рядом с другими `<script src="data/pack-...">`):

```html
<script src="data/pack-literature.js?v=32"></script>
```

3. Обнови страницу в браузере (`Ctrl+F5`). Пак появится в UI и автоматически включится.
